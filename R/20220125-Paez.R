...
ridge01 <- mountain_ridge(
  seed_num = 342, intercept = 2, n_points = 1000, steepness = "shallow")

ridge02 <- mountain_ridge(
  seed_num = 123, intercept = 3.3, n_points = 800, steepness = "shallow")
ridge02_depth <- mountain_ridge(
  seed_num = 123, intercept = 2.3, n_points = 800, steepness = "shallow")

ridge03 <- mountain_ridge(
  seed_num = 8654, intercept = 5.7, n_points = 200, steepness = "moderate")
ridge03_depth <- mountain_ridge(
  seed_num = 8654, intercept = 4.7, n_points = 200, steepness = "moderate")

ridge04 <- mountain_ridge(
  seed_num = 234, intercept = 7.5, n_points = 200, steepness = "moderate")
ridge04_depth <- mountain_ridge(
  seed_num = 234, intercept = 6, n_points = 200, steepness = "moderate")

ridge05 <- mountain_ridge(
  seed_num = 654, intercept = 12.5, n_points = 120, steepness = "steep")
ridge05_depth <- mountain_ridge(
  seed_num = 654, intercept = 11, n_points = 120, steepness = "steep")
...
selected_colours <- mex.brewer("Revolucion")
noise_gradient <- (grDevices::colorRampPalette(selected_colours[7:9]))(50)
...
p <- ggplot() +
  ...
  ggfx::with_halftone_dither(
    geom_area(
      data = ridge05, aes(x = x, y = y_nudged), fill = selected_colours[1]),
    map_size = 6) +
  ggfx::with_blur(
    geom_area(
      data = ridge05_depth, aes(x = x, y = y_nudged),
      fill = selected_colours[2], alpha = 0.8),
    sigma = 75) +
  # Ridge 4
  ggfx::with_ordered_dither(
    geom_area(
      data = ridge04, aes(x = x, y = y_nudged), fill = selected_colours[3]),
    map_size = 8) +
  ggfx::with_blur(
    geom_area(
      data = ridge04_depth, aes(x = x, y = y_nudged),
      fill = selected_colours[4], alpha = 0.8),
    sigma = 75) +
  # Ridge 3
  ggfx::with_halftone_dither(
    geom_area(
      data = ridge03, aes(x = x, y = y_nudged), fill = selected_colours[5]),
    map_size = 4) +
  ggfx::with_blur(
    geom_area(
      data = ridge03_depth, aes(x = x, y = y_nudged),
      fill = selected_colours[6], alpha = 0.8),
    sigma = 75) +
  # Ridge 2
  ggfx::with_ordered_dither(
    geom_area(
      data = ridge02, aes(x = x, y = y_nudged), fill = selected_colours[7]),
    map_size = 2) +
  ggfx::with_blur(
    geom_area(
      data = ridge02_depth, aes(x = x, y = y_nudged),
      fill = selected_colours[8], alpha = 0.8),
    sigma = 75) +
  # Ridge 1
  ggfx::with_halftone_dither(
    geom_area(
      data = ridge01, aes(x = x, y = y_nudged), fill = selected_colours[9]),
    map_size = 4) +
  ...
  labs(
    ...
    caption = "Based on code by @jacquietran") +
  theme_void() +
  theme(
...
ggsave(
  here::here("img/20220125-paez.png"),
